...
def get_valid_kafka_topic_by_prop_id(prop_id):
    db = Database()
    statement = f"SELECT * FROM tbl_node WHERE valid = TRUE AND prop->>'type' ILIKE 'kafka_topic' AND prop->>'id' ='{prop_id}'"
    print(statement)
    nodes = db.execute(statement)
    ...
    abort(404, f"Kafka topic with prop.id {id} not found")


def get_valid_kafka_topic_field_by_prop_id(prop_id):
    db = Database()
    statement = f"SELECT * FROM tbl_node WHERE valid = TRUE AND prop->>'type' ILIKE 'kafka_topic_field' AND prop->>'id' ='{prop_id}'"
    print(statement)
    nodes = db.execute(statement)
    ...
    abort(404, f"Kafka topic field with prop.id {id} not found")
