...
def update(node):
    ...
    for node_item in node:
        ...
        if prop_type is None:
            ...
            create_statement = create_statement + f"(COALESCE((SELECT prop FROM tbl_node WHERE prop->>'id'='{prop_id}'"
            create_statement = create_statement + " AND valid = FALSE ORDER BY valid_to DESC LIMIT 1)::jsonb, '{}'"
            create_statement = create_statement + f"::jsonb) || {json_prop}'::jsonb, TRUE),"
